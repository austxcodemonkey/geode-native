...
namespace apache {
  namespace geode {
    namespace client {
      ...
      void Log::init(LogLevel logLevel, const std::string& logFilename,
        ...
        try {
          if (logFilename.empty()) {
            ...
            auto diskSpaceLimit =
              logDiskSpaceLimit ? logDiskSpaceLimit : __1G__;
      ...
      std::string Log::logLevelToString(
        LogLevel level) {
      ...
      void Log::finest(const std::string& msg) {
        Log::finest(msg.c_str());
      }
      ...
      void Log::finer(const std::string& msg) {
        Log::finer(msg.c_str());
      }
      ...
      void Log::fine(const std::string& msg) {
        Log::fine(msg.c_str());
      }
      ...
      void Log::config(const std::string& msg) {
        config(msg.c_str());
      }
