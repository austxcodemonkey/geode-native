...
namespace thinclienthamixedreduncancy {
...
DUNIT_TASK_DEFINITION(CLIENT1, StepOne)
  {
    if (!g_poolConfig) {
      thinclienthamixedreduncancy::initClient(0);
    } else {
      thinclienthamixedreduncancy::initClient();
    }
    createCommRegions(0);
...
DUNIT_TASK_DEFINITION(CLIENT2, StepTwo)
  {
    if (!g_poolConfig) {
      thinclienthamixedreduncancy::initClient(1);
    } else {
      thinclienthamixedreduncancy::initClient();
    }
    createCommRegions(1);
...
DUNIT_TASK_DEFINITION(CLIENT3, StepThree)
  {
    if (!g_poolConfig) {
      thinclienthamixedreduncancy::initClient(2);
    } else {
      thinclienthamixedreduncancy::initClient();
    }
    createCommRegions(2);
