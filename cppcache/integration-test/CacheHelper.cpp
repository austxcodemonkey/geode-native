...
void _verifyEntry(const char* name, const char* key, const char* val,
  ...
  LOG(logmsg);
  _verifyEntry(name, key, val, false, false);
  LOG("Entry verified.");
}
...
void _verifyIntEntry(const char* name, const char* key, const int val,
  ...
  LOG(logmsg);
  _verifyIntEntry(name, key, val, false, false);
  LOG("Entry verified.");
}

void initClient(const bool isthinClient,
                const std::shared_ptr<apache::geode::client::Properties>& configPtr) {
...
void createPooledRegion(const char* name, bool ackMode, const char* locators,
                        const char* poolname, bool clientNotificationEnabled,
                        bool cachingEnabled) {
  LOG("createRegion_Pool() entered.");
  fprintf(stdout, "Creating region --  %s  ackMode is %d\n", name, ackMode);
  fflush(stdout);
  auto regPtr = getHelper()->createPooledRegion(name, ackMode, locators,
                                                poolname, cachingEnabled,
...
std::shared_ptr<apache::geode::client::Region> createPooledRegion(
    ...
    const std::shared_ptr<apache::geode::client::CacheListener>& listener, bool caching) {
