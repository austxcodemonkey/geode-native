...
DataOutput::DataOutput(Apache::Geode::Client::Cache ^ cache) {
  ...
  m_nativeptr = gcnew native_conditional_unique_ptr<native::DataOutput>(
      std::unique_ptr<native::DataOutput>(
          new native::DataOutput(cache->GetNative()->createDataOutput())));
  ...
  if (bytes != nullptr && bytes->Length >= 0) {
    if (len >= 0 && len <= bytes->Length) {
      ...
      throw gcnew IllegalArgumentException(
          "DataOutput::WriteBytes argument len is not in byte array range.");
    }
  } else {
    WriteByte(0xFF);
  }
...
void DataOutput::WriteSBytes(array<SByte> ^ bytes, System::Int32 len) {
  if (bytes != nullptr && bytes->Length >= 0) {
    if (len >= 0 && len <= bytes->Length) {
      ...
      throw gcnew IllegalArgumentException(
          "DataOutput::WriteSBytes argument len is not in SByte array range.");
    }
  } else {
    WriteByte(0xFF);
  }
...
void DataOutput::WriteBytesOnly(array<Byte> ^ bytes, System::UInt32 len,
  ...
  if (bytes != nullptr) {
    if (len >= 0 && len <= ((System::UInt32)bytes->Length - offset)) {
      ...
      throw gcnew IllegalArgumentException(
          "DataOutput::WriteBytesOnly argument len is not in Byte array "
          "range.");
    }
  }
}
...
void DataOutput::WriteSBytesOnly(array<SByte> ^ bytes, System::UInt32 len) {
  if (bytes != nullptr) {
    if (len >= 0 && len <= (System::UInt32)bytes->Length) {
      ...
      throw gcnew IllegalArgumentException(
          "DataOutput::WriteSBytesOnly argument len is not in SByte array "
          "range.");
    }
  }
}
...
void DataOutput::WriteCollection(System::Collections::IList ^ collection) {
  if (collection != nullptr) {
    this->WriteArrayLen(collection->Count);
          for each(Object ^ obj in collection) { this->WriteObject(obj); }
  } else
...
void DataOutput::WriteDotNetObjectArray(Object ^ objectArray) {
  ...
  String ^ pdxDomainClassname = m_cache->TypeRegistry->GetPdxTypeName(
      objectArray->GetType()->GetElementType()->FullName);
  WriteByte((int8_t)apache::geode::client::GeodeTypeIds::CacheableASCIIString);
  WriteUTF(pdxDomainClassname);
        for each(Object ^ o in list) WriteObject(o);
}
...
void DataOutput::WriteObject(Object ^ obj) {
  ...
  if (m_ispdxSerialization && obj->GetType()->IsEnum) {
    // need to set
    int enumVal = Internal::PdxHelper::GetEnumValue(
        obj->GetType()->FullName, Enum::GetName(obj->GetType(), obj),
  ...
  Byte typeId =
      m_cache->TypeRegistry->GetManagedTypeMappingGeneric(obj->GetType());
  ...
  switch (typeId) {
    ...
    case apache::geode::client::GeodeTypeIds::CacheableLinkedList: {
      ...
      this->WriteArrayLen(linkedList->Count);
          for each(Object ^ o in linkedList) this->WriteObject(o);
          return;
    }
    ...
    case apache::geode::client::GeodeTypeIds::CacheableStack: {
      CacheableStack ^ cs =
          gcnew CacheableStack((System::Collections::ICollection ^) obj);
      //  TODO: igfser mapping between generic and non generic
    ...
    default: {
      ...
      throw gcnew System::Exception(
          "DataOutput not found appropriate type to write it for object: " +
          obj->GetType());
    }
  }
}
