...
DataInput::DataInput(System::Byte* buffer, size_t size,
  ...
  if (buffer != nullptr && size > 0) {
    ...
    m_nativeptr = gcnew native_conditional_unique_ptr<native::DataInput>(
        std::unique_ptr<native::DataInput>(new native::DataInput(
            std::move(cache->GetNative()->createDataInput(buffer, size)))));
    m_cursor = 0;
...
DataInput::DataInput(array<Byte> ^ buffer,
  ...
  if (buffer != nullptr && buffer->Length > 0) {
    ...
    m_nativeptr = gcnew native_conditional_unique_ptr<native::DataInput>(
        std::unique_ptr<native::DataInput>(new native::DataInput(
            m_cache->GetNative()->createDataInput(m_buffer, len))));
...
DataInput::DataInput(array<Byte> ^ buffer, size_t len,
                     Apache::Geode::Client::Cache ^ cache) {
  m_ispdxDesrialization = false;
  m_isRootObjectPdx = false;
  m_cache = cache;
...
void DataInput::CheckBufferSize(int size) {
  if ((unsigned int)(m_cursor + size) > m_bufferLength) {
    Log::Debug("DataInput::CheckBufferSize m_cursor:" + m_cursor +
               " size:" + size + " m_bufferLength:" + m_bufferLength);
...
array<Byte> ^ DataInput::ReadBytesOnly(System::UInt32 len) {
  if (len > 0) {
    ...
    for (unsigned int i = 0; i < len; i++) bytes[i] = m_buffer[m_cursor++];
...
void DataInput::ReadBytesOnly(array<Byte> ^ buffer, int offset, int count) {
  if (count > 0) {
    ...
    for (int i = 0; i < count; i++) buffer[offset + i] = m_buffer[m_cursor++];
  }
}
...
System::Int16 DataInput::ReadInt16() { return ReadUInt16(); }

System::Int32 DataInput::ReadInt32() { return ReadUInt32(); }

System::Int64 DataInput::ReadInt64() { return ReadUInt64(); }
...
Object ^ DataInput::ReadObject() { return ReadInternalObject(); }
...
Object ^ DataInput::ReadDotNetTypes(int8_t typeId) {
  switch (typeId) {
    ...
    case apache::geode::client::GeodeTypeIds::CacheableASCIIString: {
      /*	CacheableString^ cs =
        static_cast<CacheableString^>(CacheableString::CreateDeserializable());
        cs->FromData(this);
        return cs->Value;*/
      return ReadUTF();
    }
    case apache::geode::client::GeodeTypeIds::CacheableASCIIStringHuge: {
      /*CacheableString^ cs =
      static_cast<CacheableString^>(CacheableString::createDeserializableHuge());
      cs->FromData(this);
      return cs->Value;*/
    ...
    case apache::geode::client::GeodeTypeIds::CacheableString: {
      /*CacheableString^ cs =
      static_cast<CacheableString^>(CacheableString::createUTFDeserializable());
      cs->FromData(this);
      return cs->Value;*/
      return ReadUTF();
    }
    case apache::geode::client::GeodeTypeIds::CacheableStringHuge: {
      ...
      /*CacheableString^ cs =
      static_cast<CacheableString^>(CacheableString::createUTFDeserializableHuge());
      cs->FromData(this);
      return cs->Value;*/
      return ReadUTFHuge();
    }
    ...
    case apache::geode::client::GeodeTypeIds::CacheableHashMap: {
      CacheableHashMap ^ chm = static_cast<CacheableHashMap ^>(
          CacheableHashMap::CreateDeserializable());
      chm->FromData(this);
      return chm->Value;
    }
    case apache::geode::client::GeodeTypeIds::CacheableIdentityHashMap: {
      CacheableIdentityHashMap ^ chm = static_cast<CacheableIdentityHashMap ^>(
    ...
    case apache::geode::client::GeodeTypeIds::CacheableVector: {
      /*CacheableVector^ cv =
      static_cast<CacheableVector^>(CacheableVector::CreateDeserializable());
      cv->FromData(this);
      return cv->Value;*/
      int len = ReadArrayLen();
      System::Collections::ArrayList ^ retA =
          gcnew System::Collections::ArrayList(len);
    ...
    case apache::geode::client::GeodeTypeIds::CacheableArrayList: {
      /*CacheableArrayList^ cv =
      static_cast<CacheableArrayList^>(CacheableArrayList::CreateDeserializable());
      cv->FromData(this);
      return cv->Value;*/
      int len = ReadArrayLen();
      System::Collections::Generic::List<Object ^> ^ retA =
          gcnew System::Collections::Generic::List<Object ^>(len);
      for (int i = 0; i < len; i++) {
        retA->Add(this->ReadObject());
      }
      return retA;
    ...
    case apache::geode::client::GeodeTypeIds::CacheableLinkedList: {
      ...
      return cv->Value;*/
      int len = ReadArrayLen();
      System::Collections::Generic::LinkedList<Object ^> ^ retA =
    ...
    case apache::geode::client::GeodeTypeIds::CacheableStack: {
      CacheableStack ^ cv =
          static_cast<CacheableStack ^>(CacheableStack::CreateDeserializable());
      cv->FromData(this);
      return cv->Value;
    }
    default:
...
Object ^ DataInput::ReadInternalObject() {
  try {
    ...
    if (compId == GeodeTypeIds::NullObj) {
      ...
      if (ret != nullptr) {
        Apache::Geode::Client::PdxWrapper ^ pdxWrapper =
            dynamic_cast<Apache::Geode::Client::PdxWrapper ^>(ret);
      ...
      compId = (System::Int64)classId;
               ...
               GeodeTypeIdsImpl::FixedIDByte) {  // TODO: need to verify again
    ...
    if (findinternal) {
      compId += 0x80000000;
      createType = m_cache->TypeRegistry->GetManagedDelegateGeneric(
          (System::Int64)compId);
    } else {
      ...
      if (createType == nullptr) {
        ...
        if (m_ispdxDesrialization &&
            typeId == apache::geode::client::GeodeTypeIds::
                          CacheableObjectArray) {  // object array and
                                                   // pdxSerialization
    ...
    if (createType == nullptr) {
      throw gcnew IllegalStateException("Unregistered typeId " + typeId +
                                        " in deserialization, aborting.");
    }
  ...
  if (len >= 0) {
    ...
    if (len == 0) {
      list = (System::Collections::IList ^)
                 m_cache->TypeRegistry->GetArrayObject(className, len);
      return list;
    }
    // read first object

    Object ^ ret =
        ReadObject();  // in case it returns pdxinstance or java.lang.object

    list = (System::Collections::IList ^) m_cache->TypeRegistry->GetArrayObject(
        ret->GetType()->FullName, len);
...
Object ^ DataInput::ReadInternalGenericObject() {
  ...
  if (compId == GeodeTypeIds::NullObj) {
    ...
    return Internal::PdxHelper::DeserializePdx(
        this, false,
        CacheRegionHelper::getCacheImpl(m_cache->GetNative().get())
            ->getSerializationRegistry()
            .get());
  ...
  if (findinternal) {
    compId += 0x80000000;
    createType =
        m_cache->TypeRegistry->GetManagedDelegateGeneric((System::Int64)compId);
  } else {
    createType = m_cache->TypeRegistry->GetManagedDelegateGeneric(compId);
    if (createType == nullptr) {
      Object ^ retVal = ReadDotNetTypes(typeId);
  ...
  throw gcnew IllegalStateException(
      "Unregistered typeId in deserialization, aborting.");
}
...
size_t DataInput::BytesReadInternally::get() { return m_cursor; }
...
void DataInput::AdvanceCursor(size_t offset) { m_cursor += offset; }
...
IDictionary<Object ^, Object ^> ^ DataInput::ReadDictionary() {
  ...
  if (len == -1)
    ...
    IDictionary<Object ^, Object ^> ^ dict =
        gcnew Dictionary<Object ^, Object ^>();
    for (int i = 0; i < len; i++) {
      Object ^ key = this->ReadObject();
      Object ^ val = this->ReadObject();
...
String ^ DataInput::ReadString() {
  ...
  if (typeId == GeodeTypeIds::CacheableNullString) return nullptr;
