...
PdxInstanceImpl::PdxInstanceImpl(Dictionary<String ^, Object ^> ^ fieldVsValue,
                                 PdxType ^ pdxType,
  ...
  m_pdxType = pdxType;
  m_cache = cache;
  m_cachePerfStats = &CacheRegionHelper::getCacheImpl(cache->GetNative().get())
                          ->getCachePerfStats();
  m_pdxType->InitializeType(cache);  // to generate static position map

  // need to initiailize stream. this will call todata and in toData we will
  // have stream
  auto output = new apache::geode::client::DataOutput(m_cache->GetNative()->createDataOutput());
  ...
  if (m_typeId != 0) {
    ...
    if (pdxtype == nullptr)  // will it ever happen
      throw gcnew IllegalStateException(
          "PdxType is not defined for PdxInstance: " + m_typeId);
...
Object ^ PdxInstanceImpl::GetObject() {
  ...
  Object ^ ret = Internal::PdxHelper::DeserializePdx(
      dataInput, true, m_typeId, m_bufferLength,
...
Object ^ PdxInstanceImpl::GetField(String ^ fieldName) {
  ...
  if (pft == nullptr) {
    // throw gcnew IllegalStateException("PdxInstance doesn't has field " +
    // fieldName);
    return nullptr;
  }
...
void PdxInstanceImpl::setOffsetForObject(DataInput ^ dataInput, PdxType ^ pt,
                                         int sequenceId) {
  int pos = getOffset(dataInput, pt, sequenceId);
  dataInput->ResetAndAdvanceCursorPdx(pos);
}

int PdxInstanceImpl::getOffset(DataInput ^ dataInput, PdxType ^ pt,
                               int sequenceId) {
  dataInput->ResetPdx(0);
  ...
  if (pt->NumberOfVarLenFields > 0)
    serializedLength =
        pdxSerializedLength - ((pt->NumberOfVarLenFields - 1) * offsetSize);
  else
...
bool PdxInstanceImpl::Equals(Object ^ other) {
  ...
  if (otherPdx == nullptr) return false;
  ...
  IList<PdxFieldType ^> ^ myPdxIdentityFieldList =
      getIdentityPdxFields(myPdxType);
  IList<PdxFieldType ^> ^ otherPdxIdentityFieldList =
  ...
  DataInput ^ otherDataInput =
      gcnew DataInput(otherPdx->m_buffer, otherPdx->m_bufferLength, m_cache);
  ...
  for (int i = 0; i < myPdxIdentityFieldList->Count; i++) {
    ...
    // Log::Debug("pdxfield " + ((myPFT != Default_PdxFieldType)?
    // myPFT->FieldName: otherPFT->FieldName));
    ...
    switch (fieldTypeId) {
      ...
      case PdxTypes::OBJECT: {
        ...
        if (object != nullptr) {
          if (object->GetType()->IsArray) {
            ...
            {
              if (!deepArrayEquals(object, otherObject)) return false;
            }
      ...
      case PdxTypes::OBJECT_ARRAY: {
        ...
        if (!deepArrayEquals(objectArray, otherObjectArray)) return false;
        break;
      }
      default: {
        throw gcnew IllegalStateException("PdxInstance not found typeid " +
                                          myPFT->TypeId);
      }
    }
  }
...
bool PdxInstanceImpl::compareRawBytes(PdxInstanceImpl ^ other, PdxType ^ myPT,
                                      PdxFieldType ^ myF,
                                      DataInput ^ myDataInput,
                                      PdxType ^ otherPT, PdxFieldType ^ otherF,
                                      DataInput ^ otherDataInput) {
  if (myF != Default_PdxFieldType && otherF != Default_PdxFieldType) {
    ...
    int otherPos =
        other->getOffset(otherDataInput, otherPT, otherF->SequenceId);
    int otherNextpos = other->getNextFieldPosition(
        otherDataInput, otherF->SequenceId + 1, otherPT);
    otherDataInput->ResetAndAdvanceCursorPdx(otherPos);

    if ((nextpos - pos) != (otherNextpos - otherPos)) return false;

    for (int i = pos; i < nextpos; i++) {
      if (myDataInput->ReadSByte() != otherDataInput->ReadSByte()) return false;
    }
    ...
    if (myF == Default_PdxFieldType) {
      ...
      int pos = getOffset(myDataInput, myPT, myF->SequenceId);
      int nextpos =
          getNextFieldPosition(myDataInput, myF->SequenceId + 1, myPT);
      return hasDefaultBytes(myF, myDataInput, pos, nextpos);
    }
  }
}

void PdxInstanceImpl::equatePdxFields(IList<PdxFieldType ^> ^ my,
                                      IList<PdxFieldType ^> ^ other) {
...
bool PdxInstanceImpl::deepArrayEquals(Object ^ obj, Object ^ otherObj) {
  ...
  if (!objT->Equals(otherObjT)) return false;

  if (objT->IsArray) {  // array
    return enumerableEquals((System::Collections::IEnumerable ^) obj,
                            (System::Collections::IEnumerable ^) otherObj);
  } else if (objT->GetInterface("System.Collections.IDictionary")) {  // map
    // Log::Debug(" in map");
    return enumerateDictionaryForEqual((System::Collections::IDictionary ^) obj,
                                       (System::Collections::IDictionary ^)
                                           otherObj);
  } else if (objT->GetInterface("System.Collections.IList")) {  // list
    // Log::Debug(" in list");
    return enumerableEquals((System::Collections::IEnumerable ^) obj,
                            (System::Collections::IEnumerable ^) otherObj);
  } else {
...
bool PdxInstanceImpl::enumerableEquals(System::Collections::IEnumerable ^
                                           enumObj,
  ...
  while (true) {
    ...
    if (m && o) {
      if (!my->Current->Equals(other->Current)) return false;
    } else if (!m && !o)
      return true;
    else
      return false;
...
bool PdxInstanceImpl::enumerateDictionaryForEqual(
    System::Collections::IDictionary ^ iDict,
    System::Collections::IDictionary ^ otherIDict) {
  ...
  if (iDict->Count != otherIDict->Count) return false;
...
int PdxInstanceImpl::GetHashCode() {
  ...
  for (int i = 0; i < pdxIdentityFieldList->Count; i++) {
    ...
    // Log::Debug("hashcode for pdxfield " + pField->FieldName + " hashcode is "
    // + hashCode);
    switch (pField->TypeId) {
      ...
      case PdxTypes::OBJECT_ARRAY: {
        ...
        hashCode = 31 * hashCode + (objectArray != nullptr)
                       ? deepArrayHashCode(objectArray)
                       : 0;
        break;
      }
      default: {
...
int PdxInstanceImpl::deepArrayHashCode(Object ^ obj) {
  ...
  if (objT->IsArray) {  // array
    ...
    if (obj->GetType()->Equals(DotNetTypes::BooleanType)) {
      ...
      int prime = 31;
      int h = 0;
      for (int i = 0; i < str->Length; i++) h = prime * h + str[i];
      return h;
    }
...
int PdxInstanceImpl::enumerableHashCode(System::Collections::IEnumerable ^
                                        enumObj) {
  int h = 1;
...
int PdxInstanceImpl::enumerateDictionary(System::Collections::IDictionary ^
                                         iDict) {
  int h = 0;
  System::Collections::IDictionaryEnumerator ^ dEnum = iDict->GetEnumerator();
          for each(System::Collections::DictionaryEntry ^ de in iDict) {
              // System::Collections::DictionaryEntry^ de =
              // (System::Collections::DictionaryEntry^)o;
              h = h +
                  ((deepArrayHashCode(de->Key)) ^
                   ((de->Value != nullptr) ? deepArrayHashCode(de->Value) : 0));
            }
          // Log::Debug(" in enumerateDictionary FINAL hc " + h);
          return h;
}
...
generic<class T> int PdxInstanceImpl::primitiveArrayHashCode(T objArray) {
  ...
  if (objArray->Count > 0 &&
      objArray->GetType()->GetElementType()->Equals(DotNetTypes::BooleanType))
    isBooleanType = true;
...
int PdxInstanceImpl::getRawHashCode(PdxType ^ pt, PdxFieldType ^ pField,
                                    DataInput ^ dataInput) {
  ...
  // Log::Debug("getRawHashCode nbytes " + (nextpos - pos) + " final hashcode" +
  // h);
  return h;
}

bool PdxInstanceImpl::compareDefaulBytes(DataInput ^ dataInput, int start,
                                         int end, array<SByte> ^ defaultBytes) {
  if ((end - start) != defaultBytes->Length) return false;
...
bool PdxInstanceImpl::hasDefaultBytes(PdxFieldType ^ pField,
  ...
  switch (pField->TypeId) {
    ...
    {
      throw gcnew IllegalStateException(
          "hasDefaultBytes unable to find typeID  " + pField->TypeId);
    }
  }
}
...
IList<PdxFieldType ^> ^ PdxInstanceImpl::getIdentityPdxFields(PdxType ^ pt) {
  System::Comparison<PdxFieldType ^> ^ cd =
      gcnew System::Comparison<PdxFieldType ^>(
          PdxInstanceImpl::comparePdxField);
  IList<PdxFieldType ^> ^ pdxFieldList = pt->PdxFieldList;
  List<PdxFieldType ^> ^ retList = gcnew List<PdxFieldType ^>();

  for (int i = 0; i < pdxFieldList->Count; i++) {
    PdxFieldType ^ pft = pdxFieldList[i];
    if (pft->IdentityField) retList->Add(pft);
  }
...
String ^ PdxInstanceImpl::ToString() {
  ...
  bool firstElement = true;
            for each(PdxFieldType ^ fieldType in getIdentityPdxFields(pt)) {
                ...
                result->Append("=");
                try {
                  // TODO check to see if getField returned an array and if it
                  // did use Arrays.deepToString
...
IWritablePdxInstance ^ PdxInstanceImpl::CreateWriter() {
  // dataInput->ResetPdx(0);
  return gcnew PdxInstanceImpl(
      m_buffer, m_bufferLength, m_typeId, false,
      m_cache);  // need to create duplicate byte stream
}
...
void PdxInstanceImpl::SetField(String ^ fieldName, Object ^ value) {
  ...
  if (pft != nullptr &&
      checkType(value->GetType(),
                pft->TypeId))  // TODO::need to check typeas well
  {
...
void PdxInstanceImpl::ToData(IPdxWriter ^ writer) {
  ...
  if (m_buffer != NULL) {
    ...
    if (!m_own)
      copy = apache::geode::client::DataInput::getBufferCopy(m_buffer,
                                                             m_bufferLength);

    DataInput ^ dataInput = gcnew DataInput(
        copy, m_bufferLength, m_cache);  // this will delete buffer
    dataInput->setPdxdeserialization(true);
    ...
    for (int i = 0; i < pdxFieldList->Count; i++) {
      ...
      if (value != nullptr) {  //
        ...
        if (currPf->IsVariableLengthType) {  // need to add offset
...
void PdxInstanceImpl::writeUnmodifieldField(DataInput ^ dataInput, int startPos,
  ...
  // Log::Debug("writeUnmodifieldField startpos " + startPos + " endpos " +
  // endPos);
...
int PdxInstanceImpl::getNextFieldPosition(DataInput ^ dataInput, int fieldId,
                                          PdxType ^ pt) {
  if (fieldId == pt->Totalfields) {  // return serialized length
...
Object ^ PdxInstanceImpl::readField(DataInput ^ dataInput, String ^ fieldName,
                                    int typeId) {
  switch (typeId) {
    case PdxTypes::INT: {
...
bool PdxInstanceImpl::checkType(Type ^ type, int typeId) {
  // Log::Fine("PdxInstanceImpl::checkType1 " + type->ToString() + "  " +
  // typeId);
  switch (typeId) {
    case PdxTypes::INT: {
      // Log::Fine("PdxInstanceImpl::checkType " + type->ToString() + " : "
      // +DotNetTypes::IntType->ToString());
...
void PdxInstanceImpl::writeField(IPdxWriter ^ writer, String ^ fieldName,
                                 int typeId, Object ^ value) {
  switch (typeId) {
    case PdxTypes::INT: {
